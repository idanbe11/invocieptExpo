...
export function useCellTranslate(_ref) {
  ...
  const translate = useDerivedValue(() => {
    if (!activeKey || activeIndexAnim.value < 0) return 0; // Determining spacer index is hard to visualize. See diagram: https://i.imgur.com/jRPf5t3.jpg
    ...
    if (activeIndexAnim.value < 0) return 0; // Active cell follows touch
    ...
    const shouldTranslate = isAfterActive ? cellIndex.value <= spacerIndexAnim.value : cellIndex.value >= spacerIndexAnim.value;
